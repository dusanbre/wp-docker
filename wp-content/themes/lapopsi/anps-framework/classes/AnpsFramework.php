...
class AnpsFramework
{
  ...
  public function anps_create_number_option($option_name, $number = "", $label = "", $units = "", $description = "")
  {
    ...
    $class_name = 'anps-number';

    if ($label == "") {
      $class_name .= ' anps-number-small';
    }
    ...
    $output .= "<div class='anps-number-wrap'><input class='size' type='number' name='$option_name' value='$option' id='$option_name' placeholder='$number'/><span>$units</span></div></div>";
  ...
  public function anps_create_checkbox($option, $label = "", $default = "")
  {
    ...
    $output .= "<label for='" . $option . "'>" . $label . "</label>";
    $output .= '<div class="anps-checkbox-wrap">';
   ...
   */
  public function anps_create_select($option, $optionArray, $label = '', $default_val = '', $remove_option = '0')
  {
  ...
  public function anps_get_custom_posts_array($custom_post)
  {
    $args = array(
    ...
    foreach ($posts as $item) {
      $anps_pages[$item->ID] = $item->post_title;
    }
    return $anps_pages;
  }
  ...
  public function anps_create_dummy_options($dummy_array)
  {
    ...
    foreach ($dummy_array as $dummy => $key) {
      ...
      $output .= '<footer>';
      $output .= '<input type="submit" class="dummy" name="' . $dummy . '" value="' . esc_attr__('Insert demo content', 'lapopsi') . '" />';

      if (isset($key['link'])) {
        $output .= '<a href="' . esc_url($key['link']) . '" target="_blank">' . esc_html__('Launch demo preview', 'lapopsi') . '</a>';
      }
    ...
    $output .= '<h2><strong>' . esc_html__('Import might take some time, please be patient', 'lapopsi') . '</strong></h2>';
  ...
  public function anps_create_textarea($name, $value = '', $rows = 5, $label = '')
  {
    if (!isset($name)) {
      return;
    }
  ...
  {
    $output = '';
    $output .= '<div class="fixsave"><button type="submit">' . esc_html__('Save', 'lapopsi') . '<i class="fa fa-floppy-o"></i></button></div>';
    $output .= '<button type="submit" class="bottom-save absolute"><i class="fa fa-floppy-o"></i>' . esc_html__('Save all changes', 'lapopsi') . '</button>';
    echo wp_kses($output, array(
      'div' => array(
        'class' => array(),
      ),

...
class AnpsSocial extends WP_Widget {
    public function __construct() {
        parent::__construct(
            'AnpsSocial', esc_html__('AnpsThemes - Social icons', 'anps_theme_plugin'), array('description' => esc_html__('Enter social icons to show on page', 'anps_theme_plugin'),)
        );
    ...
    function form($instance) {
        ...
        <!-- Style -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style', 'anps_theme_plugin'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('style')); ?>" name="<?php echo esc_attr($this->get_field_name('style')); ?>">
                <option value=""<?php if( esc_attr($instance['style']) == '' ) { echo ' selected'; } ?>><?php esc_html_e('Default', 'anps_theme_plugin'); ?></option>
                <option value="minimal"<?php if( esc_attr($instance['style']) == 'minimal' ) { echo ' selected'; } ?>><?php esc_html_e('Minimal', 'anps_theme_plugin'); ?></option>
                <option value="border"<?php if( esc_attr($instance['style']) == 'border' ) { echo ' selected'; } ?>><?php esc_html_e('Border', 'anps_theme_plugin'); ?></option>
                <option value="circle"<?php if( esc_attr($instance['style']) == 'circle' ) { echo ' selected'; } ?>><?php esc_html_e('Circle', 'anps_theme_plugin'); ?></option>
                <option value="circle-border"<?php if( esc_attr($instance['style']) == 'circle-border' ) { echo ' selected'; } ?>><?php esc_html_e('Circle border', 'anps_theme_plugin'); ?></option>
            </select>
        </p>

        <!-- Target -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('target')); ?>"><?php esc_html_e("Open links as", 'anps_theme_plugin'); ?></label>
        ...
        <div data-anps-repeat>
            ...
            <div class="anps-repeat-items" data-anps-repeat-items>
                ...
                <div class="anps-repeat-item" data-anps-repeat-item>
                    ...
                    <p>
                        <input placeholder="<?php esc_html_e('URL', 'anps_theme_plugin'); ?>" type="text" class="widefat" value="<?php echo esc_attr($social_url); ?>" />
                    </p>
                    <!-- Icon color -->
                    <p>
                    ...
                    <p>
                        <input class="anps-color-picker" type="text" value="<?php echo esc_attr($icon_background); ?>" />
                    </p>
    ...
    function widget($args, $instance) {
        ...
        <?php
            foreach($socials as $social) {
                ...
                if( isset($social[3]) ) {
                    $icon_color = ' style="color: ' . $social[2] . '; background-color: ' . $social[3] . '; border-color: ' . $social[3] . ';"';
                }

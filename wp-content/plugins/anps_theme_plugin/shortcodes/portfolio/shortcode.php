...
if(!function_exists('anps_portfolio_func')) {
    function anps_portfolio_func($atts, $content) {
        ...
        if ($item_button_color != '') {
            $hover_button_style = ' style="color: ' . $item_button_color . ';"';
        }
        ...
        while($portfolio_posts->have_posts()) :
            ...
            if($columns!="6" && $columns!="4") {
                ...
                $portfolio_data .= "<a class='btn btn-md'  " . $hover_button_style . " href='".get_permalink()."'>".esc_html__('Read more', 'anps_theme_plugin')."</a>";
                $portfolio_data .= "</div>";
                $portfolio_data .= "</div>";

                $portfolio_data .= "<a href='".get_permalink()."'><h3 class='project-title'" . $hover_title_style . ">".get_the_title()."</h3></a>";
            } else {
                $portfolio_data .= "<div class='projects-item-wrap'>";
                if(isset($image)&&$image!="") {
                    $portfolio_data .= $image;
